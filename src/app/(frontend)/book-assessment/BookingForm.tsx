...
export function BookingForm({ slots }: { slots: AssessmentSlot[] }) {
  ...
  return <form onSubmit={submit} className="grid items-start gap-8 lg:grid-cols-[1.15fr_.85fr]">
    ...
    <section aria-labelledby="your-details-heading" className="rounded-[2rem] bg-white p-6 shadow-sm lg:sticky lg:top-28 md:p-8"><p className="text-xs font-black uppercase tracking-[.16em] text-[#1677ff]">Step 2</p><h2 id="your-details-heading" className="mt-1 text-2xl font-black">Your details</h2><p className="mt-2 text-sm leading-6 text-[#718399]">Your answers help the coach prepare before you arrive.</p><div className="mt-6 grid gap-5"><label className="grid gap-2 text-sm font-bold">Name<input required name="playerName" maxLength={120} className="rounded-xl border border-[#092c59]/20 px-4 py-3 font-normal" /></label><label className="grid gap-2 text-sm font-bold">Email<input required name="email" type="email" maxLength={254} className="rounded-xl border border-[#092c59]/20 px-4 py-3 font-normal" /></label><label className="grid gap-2 text-sm font-bold">Phone (optional)<input name="phone" maxLength={40} className="rounded-xl border border-[#092c59]/20 px-4 py-3 font-normal" /></label><div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-2 text-sm font-bold">Playing experience<select required name="playingExperience" defaultValue="" className="rounded-xl border border-[#092c59]/20 bg-white px-4 py-3 font-normal"><option value="" disabled>Select experience</option><option value="new">New to badminton</option><option value="under-1-year">Less than 1 year</option><option value="1-3-years">1–3 years</option><option value="over-3-years">More than 3 years</option></select></label><label className="grid gap-2 text-sm font-bold">Preferred event<select required name="preferredEvent" defaultValue="" className="rounded-xl border border-[#092c59]/20 bg-white px-4 py-3 font-normal"><option value="" disabled>Select event</option><option value="singles">Singles</option><option value="doubles">Doubles</option><option value="both">Both</option><option value="not-sure">Not sure yet</option></select></label></div><label className="grid gap-2 text-sm font-bold">Main badminton goals<textarea required name="goals" maxLength={1000} rows={3} className="rounded-xl border border-[#092c59]/20 px-4 py-3 font-normal" /></label><label className="grid gap-2 text-sm font-bold">Weekly training availability<textarea required name="trainingAvailability" maxLength={500} rows={2} placeholder="Example: Tuesday and Thursday evenings" className="rounded-xl border border-[#092c59]/20 px-4 py-3 font-normal" /></label><label className="grid gap-2 text-sm font-bold">Injuries or physical limitations (optional)<textarea name="injuryConsiderations" maxLength={1000} rows={2} className="rounded-xl border border-[#092c59]/20 px-4 py-3 font-normal" /></label><label className="grid gap-2 text-sm font-bold">Anything else the coach should know? (optional)<textarea name="notes" maxLength={1000} rows={3} className="rounded-xl border border-[#092c59]/20 px-4 py-3 font-normal" /></label>{error && <p role="alert" className="text-sm font-bold text-[#b42318]">{error}</p>}<button disabled={pending || !selected} className="coach-button-primary disabled:cursor-not-allowed disabled:opacity-60">{pending ? 'Booking…' : 'Confirm assessment'}</button></div></section>
  </form>
}
