...
export default async function CoachDashboardPage() {
  ...
  const [profiles, sessions, completedSessions, progress, practices, events] = await Promise.all([
    ...
    payload.find({ collection: 'independent-practices', depth: 2, limit: 500, sort: '-updatedAt', overrideAccess: false, user }),
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-5 lg:grid-cols-12">
        ...
        <Panel className="lg:col-span-8" title="Player overview" subtitle="Program, phase, package balance, attendance, and independent practice" icon={UsersRound}>
          <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="border-b border-[#092c59]/10 text-xs uppercase tracking-wider text-[#718399]"><tr><th className="pb-3">Student</th><th className="pb-3">Program / phase</th><th className="pb-3">Package</th><th className="pb-3">Attendance</th><th className="pb-3">Practice</th></tr></thead><tbody>{profiles.docs.map((profile) => { const studentPractices = practices.docs.filter((item) => typeof item.student === 'object' && item.student.id === profile.id); const completedPractices = studentPractices.filter((item) => item.status === 'completed').length; return <tr key={profile.id} className="border-b border-[#092c59]/5 last:border-0"><td className="py-4 font-black"><Link href={`/admin/collections/student-profiles/${profile.id}`}>{profile.displayName}</Link></td><td className="py-4">{relationName(profile.program)}<span className="block text-xs text-[#718399]">{profile.currentPhase}</span></td><td className="py-4">{profile.sessionsRemaining}/{profile.packageSessions}<span className="block text-xs text-[#718399]">{profile.packageName}</span></td><td className="py-4"><span className="font-bold">{profile.attendanceRate}%</span></td><td className="py-4"><span className="font-bold">{completedPractices}/{studentPractices.length}</span><span className="block text-xs text-[#718399]">completed</span></td></tr>})}</tbody></table>{!profiles.docs.length && <Empty text="Create a student profile to begin building your roster." />}</div>
        </Panel>
