...
export const syncIndependentPractice: CollectionAfterChangeHook<StudentProfile> = async ({ doc, req }) => {
  ...
  const practice = typeof lesson.independentPractice === 'object'
    ? lesson.independentPractice as PracticeLibrary
    : await req.payload.findByID({ collection: 'practice-library', id: practiceID, depth: 1, req })
  ...
  if (existing.docs[0]) {
    ...
    await req.payload.create({
      collection: 'independent-practices',
      data: { ...practiceData, status: 'assigned' },
      depth: 0,
      overrideAccess: true,
      req,
    })
