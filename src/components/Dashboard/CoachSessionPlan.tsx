...
function DrillDetail({ drill }: { drill: string | Drill | null | undefined }) {
  ...
  return <><p className="font-black text-[#092c59]">{drill.name}</p>{skill ? <p className="mt-1 text-xs font-black uppercase tracking-wide text-[#1677ff]">Develops: {skill.name}</p> : null}<p className="mt-2">{drill.instructions}</p><p className="mt-2 text-xs"><strong>Coach for:</strong> {drill.coachingPoints}</p><p className="mt-2 text-xs font-bold text-[#1677ff]">Target: {drill.successTarget}</p></>
}
    ...
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      <div className="rounded-2xl border border-[#1677ff]/20 bg-[#eaf3ff] p-5 md:col-span-2 xl:col-span-4">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div><p className="text-xs font-black uppercase tracking-wider text-[#1677ff]">Skills developed and scored</p><p className="mt-1 text-sm leading-6 text-[#526b85]">Use every activity below to gather evidence for these lesson outcomes.</p></div>
          <div className="flex flex-wrap gap-2">{skills.length ? skills.map((skill) => <span key={skill.id} className="rounded-full bg-white px-3 py-1.5 text-xs font-black text-[#092c59] shadow-sm">{skill.name}</span>) : <span className="text-sm font-bold text-[#607286]">No lesson skills selected.</span>}</div>
        </div>
      </div>
