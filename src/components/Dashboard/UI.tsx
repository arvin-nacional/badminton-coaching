...
export function Panel({ children, className = '', title, subtitle, icon: Icon }: { children: React.ReactNode; className?: string; title: string; subtitle?: string; icon?: LucideIcon }) {
  return <section className={`rounded-[1.6rem] border border-[#092c59]/10 bg-white p-6 shadow-[0_12px_40px_-30px_rgba(9,44,89,.35)] ${className}`}><div className="mb-5 flex items-start justify-between gap-4"><div><h2 className="text-lg font-black">{title}</h2>{subtitle && <p className="mt-1 text-sm text-[#718399]">{subtitle}</p>}</div>{Icon && <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#eaf3ff] text-[#1677ff]"><Icon className="h-5 w-5" /></span>}</div>{children}</section>
}

export function Stat({ label, value, detail }: { label: string; value: string | number; detail?: string }) {
  return <div className="rounded-2xl bg-[#f3f7fc] p-4"><p className="text-xs font-bold uppercase tracking-[.12em] text-[#718399]">{label}</p><p className="mt-2 text-2xl font-black">{value}</p>{detail && <p className="mt-1 text-xs text-[#718399]">{detail}</p>}</div>
}
