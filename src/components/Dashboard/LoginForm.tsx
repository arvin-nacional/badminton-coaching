          ...
          <>
            <div className="mt-7">
              <GoogleAuthButton
                ...
                intent="signin"
                onAuthenticated={(user) => finishLogin(user.roles)}
              />
            </div>
            <div className="my-6 flex items-center gap-3 text-xs font-bold uppercase tracking-[.14em] text-[#8a9aad]">
              <span className="h-px flex-1 bg-[#d9e1ea]" /> or use email <span className="h-px flex-1 bg-[#d9e1ea]" />
            </div>
          </>
        )}

        <form className={googleClientID ? 'space-y-5' : 'mt-7 space-y-5'} onSubmit={login}>
